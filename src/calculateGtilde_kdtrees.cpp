...
int main(int argc, char* argv[])
{
  ...
  g3lcong::Kdtree sources(filename_sources, 6, 1, 2, 5, 3, 4, 6);
  ...
  for(int i=0; i<num_bins; i++)
      ...
      for(int j=0; j<num_bins; j++)
	{
	  ...
	  for(int k=0; k<num_bins; k++)
		...
		<< three_pt_stat.G_real_total_.at(i*num_bins*num_bins
						  + j*num_bins + k)<<" "	//Real part of Gtilde [dimensionless or Msun/Mpc²]
		<< three_pt_stat.G_imag_total_.at(i*num_bins*num_bins
						  + j*num_bins + k)<<" "	//Imaginary part of Gtilde [dimensionless or Msun/Mpc²]
		<< three_pt_stat.weight_total_.at(i*num_bins*num_bins
						  + j*num_bins + k)		// Weight of Gtilde [dimensionless or Msun/Mpc²]
		<<std::endl;
	    };
	};
    };
