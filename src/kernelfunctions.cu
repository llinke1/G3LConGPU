...
__global__ void g3lcong::addToGGE(double *x1, double *y1, double *x2, double *y2, double *xS, double *yS,
								  double *chi1, double *chi2, double *chiS, double *e1, double *e2, double *w,
								  double deltaChi, double num_bins, int N1, int N2, int NS, double theta_min, double theta_max,
								  double *Greal, double *Gimag, double *weight)
{
	...
	for (int k = thread_index; k < NS; k += blockDim.x * gridDim.x)
	{
		...
		for (int i = 0; i < N1; i++)
		{
			...
			double x_gal1 = x1[i];
			double y_gal1 = y1[i];
			double chi_gal1 = chi1[i];
			if (abs(chi_gal1 - chi_galS) > deltaChi)
				continue;
			...
			for (int j = 0; j < N2; j++)
			{
				...
				double x_gal2 = x2[j];
				double y_gal2 = y2[j];
				double chi_gal2 = chi2[j];
				if (abs(chi_gal2 - chi_galS) > deltaChi)
					continue;

				double dx2 = x_galS - x_gal2; // x-distance betw lens2 & source
				double dy2 = y_galS - y_gal2; // y-distance betw lens2 & source

				double phi2 = atan2(dy2, dx2);
				if (phi2 < 0)
					phi2 += 2 * g3lcong::pi;


				double dx = x_gal1 - x_gal2;
				double dy = y_gal1 - y_gal2;
				double r = sqrt(dx * dx + dy * dy);

				// Get index for Gtilde in logarithmic binning
				int indexR = 0;
				if (r > theta_min)
					indexR = floor(log(r / theta_min) / theta_binwidth);

				int indexPhi1 = floor(0.5 * phi1 * num_bins / g3lcong::pi);
				int indexPhi2 = floor(0.5 * phi2 * num_bins / g3lcong::pi);

				unsigned int index = indexR * num_bins * num_bins + indexPhi1 * num_bins + indexPhi2;

				if (indexR < num_bins && indexPhi1 < num_bins && index < num_bins * num_bins * num_bins)
				{
					...
					double Greal_triplet = (-eps1 * cos_phase - eps2 * sin_phase) * w_galS * 0.01;
					double Gimag_triplet = (eps1 * sin_phase - eps2 * cos_phase) * w_galS * 0.01;

					// Add Triplet contribution
					atomicAdd(&Greal[index], Greal_triplet);
					atomicAdd(&Gimag[index], Gimag_triplet);
					atomicAdd(&weight[index], w_galS * 0.01);
				}
			}
		}
	}
...
__global__ void g3lcong::addToGE(double *x1, double *y1, double *xS, double *yS,
								 double *chi1, double *chiS, double *e1, double *e2, double *w,
								 double deltaChi, double num_bins, int N1, int NS, double theta_min, double theta_max,
								 double *Greal, double *Gimag, double *weight)
{
	...
	for (int k = thread_index; k < NS; k += blockDim.x * gridDim.x)
	{
		...
		for (int i = 0; i < N1; i++)
		{
			...
			double dx = x_galS - x_gal1; // x-distance betw lens1 & source
			double dy = y_galS - y_gal1; // y-distance betw lens1 & source
			double r = sqrt(dx * dx + dy * dy);
			double phi = atan2(dy, dy);
			...
			{
				double cos_phase, sin_phase;
				sincos(phi, &sin_phase, &cos_phase);
				...
				// Add Triplet contribution
				atomicAdd(&Greal[indexR], Greal_triplet);
				atomicAdd(&Gimag[indexR], Gimag_triplet);
				atomicAdd(&weight[indexR], w_galS * 0.01);
			}
		}
	}
}
		...
		for (int i = 0; i < N1; i++)
		{
			...
			for (int j = 0; j < N2; j++)
			{
				...
				if (indexX < num_bins && indexY < num_bins && index < num_bins * num_bins * num_bins)
				{
					...
					if (r2 > omega_theta_min * omega_theta_min)
						index_omega = int((0.5 * log(r2 / omega_theta_min / omega_theta_min)) / om_binwidth);

					double omega_triplet = 0; // omega for this lens pair
					if (index_omega < num_bins)
						omega_triplet = omega[index_omega];
	...
	for (int k = thread_index; k < NS; k += blockDim.x * gridDim.x) // check that we do not go outside of submatrix border
	{
		...
		for (int i = 0; i < N1; i++)
		{
			...
			for (int j = 0; j < N2; j++)
			{
				...
				int indexX = 0;
				if (r1 > r_min)
					indexX = floor(log(r1 / r_min) / r_binwidth);
				...
				if (index < num_bins * num_bins * num_bins && indexX < num_bins && indexY < num_bins && phi < 2 * g3lcong::pi && SigCrit_triplet > 0)
				{
					...
					double Greal_triplet = (1 + omega_triplet) * (-eps1 * cos_phase - eps2 * sin_phase) * w_galS * weightZ / SigCrit_triplet;
					double Gimag_triplet = (1 + omega_triplet) * (eps1 * sin_phase - eps2 * cos_phase) * w_galS * weightZ / SigCrit_triplet;
										...
										double sigma2, double *paircount)
{
	...
	for (int i = thread_index; i < N1; i += blockDim.x * gridDim.x)
	{
		...
		for (int j = 0; j < N2; j++)
		{
			...
			if (r2 > 0)
				index = int((0.5 * log(r2) - log(theta_min)) / binwidth);
...
__global__ void g3lcong::addToTriplecount(double *x1, double *y1, double *z1, double *x2, double *y2, double *z2, double *x3, double *y3, double *z3, int N1, int N2, int N3, int num_bins, double r_min, double r_binwidth, double *Dcom, double z_min, double z_binwidth, int *triplecount)
{
	...
	for (int i = thread_index; i < N1; i += blockDim.x * gridDim.x)
	{
		...
		double xgal1 = d1 * x1[i] * 0.000290889; // Physical x coordinate [Mpc], Factor is conversion to Rad
		double ygal1 = d1 * y1[i] * 0.000290889; // Physical y coordinate [Mpc], Factor is conversion to Rad

		for (int j = 0; j < N2; j += blockDim.x * gridDim.x)
		{
			double zgal2 = z2[j];					   // redshift
			...
			if (index_rp12 > 0 && index_pi12 > 0 && index_rp12 < num_bins && index_pi12 < num_bins)
			{

				for (int k = 0; k < N3; k += blockDim.x * gridDim.x)
				{
					double zgal3 = z3[k];					   // redshift
					index_z = int(zgal3 - z_min / z_binwidth); // index of redshift
					if (index_z > num_bins)
						continue;
					double d3 = Dcom[index_z];				 // Comoving distance to galaxy [Mpc]
					double xgal3 = d3 * x3[k] * 0.000290889; // Physical x coordinate [Mpc]
					double ygal3 = d3 * y3[k] * 0.000290889; // Physical y coordinate [Mpc]
					...
					if (index_rp13 > 0 && index_rp23 > 0 && index_pi23 > 0 && index_rp13 < num_bins && index_rp23 < num_bins && index_pi23 < num_bins)
					{

						int index = index_rp12 * num_bins * num_bins * num_bins * num_bins + index_rp13 * num_bins * num_bins * num_bins + index_rp23 * num_bins * num_bins + index_pi12 * num_bins + index_pi23;
						if (index > 0 && index < num_bins * num_bins * num_bins * num_bins * num_bins)
						{
