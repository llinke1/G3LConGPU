...
int g3lcong::readPhysSources(std::string filename, const int &total_columns,
...

{
  ...
  if (input.fail())
  {
    std::cerr << "readPhysSources: Couldn't open " << filename << std::endl;
    return 1;
  };
  ...
  while (getline(input, line))
  {
    ...
    e1.push_back(values.at(col_e1 - 1));
    e2.push_back(values.at(col_e2 - 1));
    weight.push_back(values.at(col_weight - 1));
    DA.push_back(values.at(col_DA - 1));
  };
...
int g3lcong::readPhysLenses(std::string filename, const int &total_columns,
