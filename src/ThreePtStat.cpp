...
#pragma omp barrier //Wait till all jobs are finished
  ...
  // Combine results from threads
   for(int i=0; i<num_bins_*num_bins_*num_bins_; i++)
	...
	{
	  if(total_weight + weight_list.at(j).at(i)!=0
	     && weight_list.at(j).at(i)!=0)
	    {
	      G_real_total_.at(i)=
		G_real_total_.at(i)*total_weight
		/(total_weight + weight_list.at(j).at(i))
		+ G_real_list.at(j).at(i)*weight_list.at(j).at(i)
		/(total_weight + weight_list.at(j).at(i));

	      G_imag_total_.at(i)=
		G_imag_total_.at(i)*total_weight
		/(total_weight + weight_list.at(j).at(i))
		+ G_imag_list.at(j).at(i)*weight_list.at(j).at(i)
		/(total_weight + weight_list.at(j).at(i));

	      theta1_com_total_.at(i)=
		theta1_com_total_.at(i)*total_weight
		/(total_weight + weight_list.at(j).at(i))
		+ theta1_com_list.at(j).at(i)*weight_list.at(j).at(i)
		/(total_weight + weight_list.at(j).at(i));

	      theta2_com_total_.at(i)=
		theta2_com_total_.at(i)*total_weight
		/(total_weight + weight_list.at(j).at(i))
		+ theta2_com_list.at(j).at(i)*weight_list.at(j).at(i)
		/(total_weight + weight_list.at(j).at(i));

	      phi_com_total_.at(i)=
		phi_com_total_.at(i)*total_weight
		/(total_weight + weight_list.at(j).at(i))
		+ phi_com_list.at(j).at(i)*weight_list.at(j).at(i)
		/(total_weight + weight_list.at(j).at(i));
					   ...
					   std::vector<double>& phi_com)
{
  ...
  if (lens1==NULL || lens2==NULL || source==NULL) return;
  ...
  if(a<theta_min_ || a>theta_max_ || b<theta_min_ || b>theta_max_) return;
  ...
  unsigned long index =
    floor(log(a/theta_min_)/theta_binsize_) *num_bins_*num_bins_
    + floor(log(b/theta_min_)/theta_binsize_)*num_bins_
    + floor(phi/phi_binsize_);
  ...
  double nw =
    ow +  source->central_gal_.weight_
    * lens1->central_gal_.weight_
    * lens2->central_gal_.weight_; ///<new weight
  ...
  if(nw != 0 && index<G_real.size()) // Ignore if weight is zero
    {
      theta1_com.at(index) = (theta1_com.at(index)*ow/nw
			      + a*(source->central_gal_.weight_
				   * lens1->central_gal_.weight_
				   * lens2->central_gal_.weight_)/nw);
      theta2_com.at(index) = (theta2_com.at(index)*ow/nw
			      + b*(source->central_gal_.weight_
				   * lens1->central_gal_.weight_
				   * lens2->central_gal_.weight_)/nw);
      phi_com.at(index) = (phi_com.at(index)*ow/nw
			   + phi*(source->central_gal_.weight_
				  * lens1->central_gal_.weight_
				  * lens2->central_gal_.weight_)/nw);
					   ...
					   std::vector<double>& phi_com)
{
  ...
  unsigned long index =
    floor(log(D*a*g3lcong::rad_in_arcmin/r_min_)/r_binsize_) *num_bins_*num_bins_
    + floor(log(D*b*g3lcong::rad_in_arcmin/r_min_)/r_binsize_)*num_bins_
    + floor(phi/phi_binsize_);
  ...
  double nw_sigCrit =
    ow_sigCrit +  source->central_gal_.weight_
    * lens1->central_gal_.weight_
    * lens2->central_gal_.weight_
    / sig_crit / sig_crit; ///<new weight
  ...
  if(nw_sigCrit != 0 && index<G_real.size()) // Ignore if weight is zero
    {
      r1_com.at(index) = (r1_com.at(index)*ow_sigCrit/nw_sigCrit
			      + D*a*g3lcong::rad_in_arcmin
			  *(source->central_gal_.weight_
			    * lens1->central_gal_.weight_
			    * lens2->central_gal_.weight_)/nw_sigCrit);
      r2_com.at(index) = (r2_com.at(index)*ow_sigCrit/nw_sigCrit
			      + D*b*g3lcong::rad_in_arcmin*(source->central_gal_.weight_
				   * lens1->central_gal_.weight_
				   * lens2->central_gal_.weight_)/nw_sigCrit);
      phi_com.at(index) = (phi_com.at(index)*ow_sigCrit/nw_sigCrit
			   + phi*(source->central_gal_.weight_
				  * lens1->central_gal_.weight_
				  * lens2->central_gal_.weight_)/nw_sigCrit);
	

      G_real.at(index) = G_real.at(index)*ow_sigCrit/nw_sigCrit
	+ real(c_sigCrit)/nw_sigCrit;
      G_imag.at(index) = G_imag.at(index)*ow_sigCrit/nw_sigCrit
	+ imag(c_sigCrit)/nw_sigCrit;
