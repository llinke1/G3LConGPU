	...
	for (int k = thread_index; k < NS; k += blockDim.x * gridDim.x)
	{
		...
		for (int i = 0; i < N1; i++)
		{
			...
			for (int j = 0; j < N2; j++)
			{
				...
				if (theta2 > theta_max || theta2 < theta_min)
					continue;
										  ...
										  double *weight)
{
	...
	for (int k = thread_index; k < N1; k += blockDim.x * gridDim.x)
	{
		...
		for (int i = 0; i < N2; i++)
		{
			...
			// Get positions of source 2 in arcmin
			double x_gal2 = x2[i];
			double y_gal2 = y2[i];
			...
			// Go through all lens galaxies in submatrix
			for (int j = 0; j < NL; j++)
			{
				...
				if (indexX < num_bins && indexY < num_bins && index < num_bins * num_bins * num_bins_phi)
				{
					...
					*/
					...
					atomicAdd(&Gplus_imag[index], weight1 * weight2 * Gplus_imag_triplet * 0.01);
					atomicAdd(&Gminus_real[index], weight1 * weight2 * Gminus_real_triplet * 0.01);
...
__global__ void g3lcong::addToGtildeIABiased(double *x1, double *y1, double *z1, double *x2, double *y2, double *z2,
...
__global__ void g3lcong::addToGplusGminusIA(double *xL, double *yL, double *zL,
											double *x1, double *y1, double *z1,
											double *x2, double *y2, double *z2,
											double *e11, double *e21, double *e12, double *e22,
											double Pi, int NL, int N1, int N2, double r_min, double r_max, int num_bins,
											double *Gplus_real, double *Gplus_imag,
											double *Gminus_real, double *Gminus_imag,
											double *weight)
{
	...
	for (int k = thread_index; k < N1; k += blockDim.x * gridDim.x)
	{
		...
		cuDoubleComplex shear1 = make_cuDoubleComplex(eps11, eps21);

		for (int i = 0; i < NL; i++)
		{

			// Get positions of lens
			double x_galL = xL[i];
			double y_galL = yL[i];
			double z_galL = zL[i];

			double dx1 = x_galL - x_gal1; // x-distance betw lens & source1
			double dy1 = y_galL - y_gal1; // y-distance betw lens & source1
			double dz1 = z_galL - z_gal1;
			...
			for (int j = 0; j < N2; j++)
			{
				...
				// Get positions of source 2
				double x_gal2 = x2[j];
				double y_gal2 = y2[j];
				double z_gal2 = z2[j];

				double dx2 = x_galL - x_gal2; // x-distance betw lens & source2
				double dy2 = y_galL - y_gal2; // y-distance betw lens & source2
				double dz2 = z_galL - z_gal2;
				...
				double eps12 = e12[j];
				double eps22 = e22[j];
				cuDoubleComplex shear2 = make_cuDoubleComplex(eps12, eps22);
				...
				// atan2 returns angle in [-pi, pi], we want [0,2pi]
				if (phi < 0)
					phi += 2 * 3.1416; // g3lcong::pi;

				unsigned int index = indexX * num_bins * num_bins + indexY * num_bins + floor(0.5 * phi * num_bins / 3.1416);
				...
				// Add Triplet contribution
				atomicAdd(&Gplus_real[index], Gplus_real_triplet * 0.01);
				atomicAdd(&Gplus_imag[index], Gplus_imag_triplet * 0.01);
				atomicAdd(&Gminus_real[index], Gminus_real_triplet * 0.01);
				atomicAdd(&Gminus_imag[index], Gminus_imag_triplet * 0.01);
				atomicAdd(&weight[index], 0.01);
			}
		}
	}
