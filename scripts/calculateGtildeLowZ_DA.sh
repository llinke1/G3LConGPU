...
then
    ...
    BIN_GTILDE=$DIR_BIN/calculateGtilde_gpu_IA_DA.x
	echo $BIN_GTILDE
	echo $SIGMA
else
...
do
	...
	echo ">Gtilde for $tile | $(date)"
       ...
       $BIN_GTILDE $DIR_DATA1/$tile.$NAME_SOURCES.dat $DIR_DATA1/$tile.$NAME_OBJECTS.dat $DIR_DATA2/$tile.$NAME_OBJECTS.dat $FILE_OMEGA $THETA_MIN $THETA_MAX $R_MIN $R_MAX $NBINS $SIGMA $FILE_SIGMACRIT $FILE_DA $IS_PHYS  > $DIR_PRODUCTS/gtilde/$tile.gtilde_single_DA.dat
    fi
done
...
then
  ...
  python $DIR_PYTHON/combineGtilde.py 0 $NTILES $DIR_PRODUCTS/gtilde/all.gtilde_DA.dat $DIR_PRODUCTS/gtilde/?.gtilde_single_DA.dat $DIR_PRODUCTS/gtilde/??.gtilde_single_DA.dat $DIR_PRODUCTS/gtilde/???.gtilde_single_DA.dat
fi
...
then
	...
	do
		...
		python $DIR_PYTHON/combineGtilde.py $I $NTILES $DIR_PRODUCTS/gtilde/jn_$I.gtilde_DA.dat $DIR_PRODUCTS/gtilde/*.gtilde_single_DA.dat &
	    fi
	    ((NUMBER_JOBS++))
	    ((I++))
