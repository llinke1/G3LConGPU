...
print("Len Gtilde FN:", len(fns_gtilde))
...
G_real=np.zeros_like(theta1)
G_imag=np.zeros_like(theta1)
weight=np.zeros_like(theta1)

for fn_gtilde in fns_gtilde:
    data=np.loadtxt(fn_gtilde)
    G_real+=(data[:,6]*data[:,8])
    G_imag+=(data[:,7]*data[:,8])
    weight+=(data[:,8])
